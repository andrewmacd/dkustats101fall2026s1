...
webr:
  ...
  packages:
    ...
    - ggrepel
  resources:
    - https://raw.githubusercontent.com/andrewmacd/dkustats101fall2026s1/main/lectures/datasets/Country_data.csv
...
```{webr}
ggplot(global.data.augmented, aes(x=gdp.capita, y=hdi.score)) +
  geom_point(aes(color = gdp.capita > 10)) +  # color conditionally
  scale_color_manual(values = c("FALSE" = "black", "TRUE" = "red")) +
  geom_text_repel( data = subset(global.data, gdp.capita > 10),  
    aes(label = Country),
    vjust = -0.5,
    color = "red") +
  labs(x="GDP per capita (10000 USD) in PPP terms", y="Human Development Index (HDI) score") 
...
What do you think the $R^2$ will change to when we remove the outliers?
...
ggplot(global.data.log, aes(x=log.gdp.capita, y=hdi.score)) +
  geom_point(aes(color = log.gdp.capita < -5)) +  # color conditionally
  scale_color_manual(values = c("FALSE" = "black", "TRUE" = "red")) +
  geom_text(
    data = subset(global.data.log, log.gdp.capita < -5),  # only label those < -6
    aes(label = Country),
    vjust = -0.5,
    color = "red"
  ) + 
...
global.data.log %>% 
  filter(log.gdp.capita < -5) %>% 
  mutate(`World Bank PPP GDP estimate` = World.Bank.Forecast.GDP.PPP.*10000,
         `Population 2023` = Population.2023,
         `GDP per capita` = gdp.capita*10000) %>% 
